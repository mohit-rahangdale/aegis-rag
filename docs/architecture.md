...
### 4.9 Production Engineering, Evaluation & CI/CD Architecture

* **Problem Solved**: Lack of empirical quality metrics (silent retrieval degradation, hallucinated answers) and manual, error-prone deployment.
* **Key Design Decisions**:
  1. **Automated RAG Evaluation Harness (`app/evaluation/`)**:
     * **Recall@K**: Quantifies the percentage of gold-standard relevant chunks returned in top K candidates.
     * **Context Precision (MAP)**: Rewards pipelines that rank relevant passages at the very top of the retrieved set.
     * **Faithfulness / Grounding Score**: Evaluates whether statements generated by the model are directly supported by context.
  2. **Golden Benchmark Dataset**:
     * Curated multi-domain evaluation samples testing architecture knowledge, multi-hop reasoning, and guardrail edge cases.
  3. **Multi-Stage Production Docker Build**:
     * Separates the build dependencies from the final minimal runtime image.
     * Runs as a non-privileged `appuser` with integrated container health checks.
  4. **Automated CI/CD Pipeline (GitHub Actions)**:
     * Validates dependencies, code compilation, and runs the entire automated test suite on every pull request and push to `main`.
